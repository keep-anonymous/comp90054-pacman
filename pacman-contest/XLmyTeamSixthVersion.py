...
''' Author is Xinjie Lan
    this is the sixth version for offensive agent using approximate Q learning technique
    After watched the replay, I found that the escape plan was not good. If there was a ghost on the way back
    the pacman run into it. This version tried to avoid the ghost by using reverse. Also, the order of defining back feature
    is adjust and caplusel is more likely to get. 

    Since reward is very hard to define. I simply used get food and scores to define the reward. 

    
    there are some future works 
                                1 Double check with the escape plan. This version didn't test many senarios.
                                2 reward function can be studied more
                                3 possible potential-based reward shaping method
                                4 reconstruct the code
                                5 Consider the tradeoff when carrying food vs time and depth of enemy territory
                                6 Consider the equiblirum state when pacman and ghost fighting on boarders 
  ...
  def getFeatures(self, gameState, action):
    ...
    if enemiesInvisible:
      ...
      if isScared:
        ...
        features['eatGhost'] = float(distanceToGhost)
        features['back'] = 1.0
      else:
        ...
        if gameState.getAgentState(self.index).isPacman:
          ...
          if min(self.getMazeDistance(myPosition, ghostPos) for ghostPos in enemyGhostPosition) <6:
            features['back'] = -float(self.getMazeDistance(myPosition, InitialPosition))/(walls.width * walls.height)
          #features['back'] = float(self.getMazeDistance(myPosition, InitialPosition))/(walls.width * walls.height)
