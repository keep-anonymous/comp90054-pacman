...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.weights = {'carrying':0.0, 'successorScore': 33.799511273486225, 'getFood': 0.0
                    , 'getCaplual': -5.091081441155061, 'enemyOneStepToPacman': 2.8453693171749803, 'towardToGhost': 0.0,'distanceToFood': -10.608110698692535,
                    'back': 17.887596441151473, 'stop': 0.0, 'eatGhost':-20.34059388815152, 'reverse': -5.120089541629367}
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
      ...
      if len(invaders) == 0:
        ...
        if len(nearOpponents) > 0 and len(self.defendingFood)<=len(myFood):
