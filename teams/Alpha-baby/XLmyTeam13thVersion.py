...
''' The author of attacking agentis XINJIE LAN and the author of defensive agent is LIUYI CHAI
    This is the 13th version for offensive agent using approximate Q learning technique.
    This version fixed the issue that the method of astar will be called twice due to aSt attribute didn't change
    Also, a feature that don't eat corner food when enemy one step nearby added.
    when eat capulse, pacman will first attack then get as food as he can then go back
    ...
    there are some future works  very first one ! corner food need to be solved 
...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.weights = {'carrying':-0.4455122196160773, 'successorScore': 37.62979415520068, 'getFood': 0.0
                    , 'getCaplual': -5.988883482413958, 'enemyOneStepToPacman': 2.9516606995155596, 'towardToGhost': 0.0,'distanceToFood': -11.03416809526844,
                    'back': 19.330711749842703, 'stop': 0.0, 'eatGhost':-20.27122067665751, 'reverse': -5.314805655604604}
      ...
      if isScared:
        ...
        if gameState.getAgentState(self.index).isPacman:
              ...
              if action == rev and currentCarry !=0:
                ...
                '''for food in foodList:
                  ...
                  if myPosition == food:
                    ...
                    if (foodX+1,foodY) in walls:
                      wallCount +=1
                    if (foodX-1,foodY) in walls:
                      wallCount += 1
                    if (foodX,foodY+1) in walls:
                      wallCount += 1
                    if (foodX-1,foodY-1) in walls:
                      wallCount += 1
                    if wallCount>=3:
                      features['distanceToFood'] = 0
                      features['successorScore'] = 0'''
            ...
            if minFoodDistance[0] <4 and minFoodDistance[1] not in otherFood  and distanceToEnemyGhost <=2 and action != Directions.STOP :
              ...
              wallCount = 0
              #print'nearest FOOD',minFoodDistance[1]
              '''for food in foodList:
                foodX, foodY = food
                wallCount = 0
