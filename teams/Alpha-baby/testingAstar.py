...
''' Author is Xinjie Lan
    This version mainly solve the equliburim state, which is a state that my pacman
    and enemy ghost stuck on the border. No one moves.
    ...
    No bugs found so far! Complete Version
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    dx, dy = Actions.directionToVector(action)
    xAfterMove, yAfterMove = int(x + dx), int(y + dy)
    self.nextPositionGlo = (xAfterMove,yAfterMove)
    ghostPositions = []
    ...
    if enemiesInvisible:
      ...
      if isScared:
        ...
        if gameState.getAgentState(self.index).isPacman:
          ...
          if min([self.getMazeDistance(myCurrentPos, ghostPos) for ghostPos in enemyGhostPosition]) <4:
            otherSideDis = []
            '''for point in border:
              otherSideDis.append((self.getMazeDistance(myCurrentPos, point),point))
            otherSide = max(otherSideDis)'''
            #pointDis = self.aStar(gameState,action,otherSide[1])
            #print'pointDis',pointDis
          
            #features['getFood'] = pointDis
            #if not self.skipAstar:
            #print'otherSide',otherSide
            #self.farPoint = otherSide[1]
